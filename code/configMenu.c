...
{
    int BTN_HEIGHT = (SCREEN_HEIGHT - marginFromTitle) / (CONFIGURATIONS_OPTIONS_QUANTITY / 2);
    int BTN_WIDTH = SCREEN_WIDTH / 5;
    ...
    configurationsOptionsRec[CONFIGURATIONS_BTN_GOBACK] = (Rectangle){.x = 0, .y = configurationsOptionsRec[0].y, .width = BTN_WIDTH, .height = BTN_HEIGHT};
}