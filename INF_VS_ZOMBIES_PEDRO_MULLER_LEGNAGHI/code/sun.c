...
double spawnRateSun = STANDART_SPAWN_RATE_SUN;
...
{
    if (indexOfSunRemoved < 0 || indexOfSunRemoved >= *indexOfNextSun)
        return; // checking if valid index
...
void collectSun(Rectangle array_of_suns[SIZE_OF_SUN_ARR], int *indexOfNextSun, float groundOfTheSuns[SIZE_OF_SUN_ARR])
{
    for (int i = 0; i < *indexOfNextSun; i++)
    {
        ...
        {                                                                        // if mouse of player and the sun collided
...
void DrawSuns(Rectangle array_of_suns[SIZE_OF_SUN_ARR], int indexOfNextSun, Texture2D sunTexture)
{
    ...
    for (int i = 0; i < indexOfNextSun; i++)
    {
        ...
        { // if the sun is not an invalid sun
